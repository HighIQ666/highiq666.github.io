...
- ②偶次根式（被开方数≥0）；
- ③$x^0$（$x≠0$）；
- ④$y=log_a x$（$a>0$且$a≠1$，$x>0$）；
- ⑤$y=a^x$（$a>0$且$a≠1$，$\in R$）；
...
1. 定义域对称；
2. 特殊值，如奇函数$f(0)=0$，$f(1)\text{与}f(-1)$，需要验证；
...
2. 性质：$(\sqrt[n]{a})^n=a$；$\sqrt[n]{a^n}=\begin{cases}a,n\text{为奇数}\\|a|,n\text{为偶数}\end{cases}$；$x^0=1(x≠0)$；
...
1. **定义**：形如$y=a^x(a>0\text{且}a≠1)$的函数为指数函数。
...
1. **定义**：形如$y=log_a x(a>0\text{且}a≠1)$的函数叫对数函数。
